...
**Status:** ✅ PHASE 4 COMPLETE | 📋 PHASE 5 PLANNED | 📋 PHASE 6 DEFERRED
...
## Phase 5: Code Refactoring 📋 PLANNED

**Timeline:** TBD (estimated 2-3 hours implementation)
**Objective:** Reduce `server.py` from 3,124 lines to ~1,100 lines (**65% reduction**)
**Status:** Proposed

### Problem Statement

The current `server.py` is 3,124 lines with significant code duplication across 24 tools:

| Metric | Current Value |
|--------|---------------|
| Total lines | 3,124 |
| Tools | 24 |
| Avg lines per search tool | ~80 |
| Avg lines per service tool | ~60 |
| Duplicated patterns | 6 major patterns |

### Identified Repetitive Patterns

#### Pattern 1: Tool Annotations (120 lines saved)
Every tool repeats identical `ToolAnnotations`:
```python
@server.tool(
...
#### Pattern 2: OC Resolution (~140 lines saved)
Every tool repeats session config extraction:
```python
...
#### Pattern 3: Error Handling (~300 lines saved)
Every tool has identical try/except blocks:
```python
...
#### Pattern 4: ID Stringification (~48 lines saved)
Service tools repeat ID conversion:
```python
if id is not None: id = str(id)
if mst is not None: mst = str(mst)
```

#### Pattern 5: Search Tool Logic (~320 lines saved)
8 search tools share: OC resolution → date validation → param building → ranking → API call

#### Pattern 6: Service Tool Logic (~300 lines saved)
10 service tools share: ID stringify → validation → OC resolution → param building → API call

### Proposed Solution: Hybrid Helper Approach

Create `src/lexlink/_helpers.py` (~250 lines) with extracted common logic, keeping explicit tool definitions in `server.py`.

**Why Hybrid?**
- **MCP Safe:** Keeps explicit `@server.tool()` decorators
- **Smithery Safe:** Preserves `ctx: Context = None` parameters
- **PlayMCP Safe:** `slim_response()` unchanged
- **LLM Safe:** Docstrings and type hints remain visible

### New File Structure

```
src/lexlink/
├── server.py       # 3,124 → ~1,100 lines (65% reduction)
├── _helpers.py     # NEW: ~250 lines (shared logic)
└── ... (other files unchanged)
```

### Helper Module Contents

| Helper | Purpose | Lines Saved |
|--------|---------|-------------|
| `READ_ONLY_TOOL` | Shared ToolAnnotations constant | 5 × 24 = 120 |
| `get_session_oc(ctx)` | Extract OC from context | 3 × 24 = 72 |
| `with_error_handling(name)` | Decorator for try/except | 12 × 24 = 288 |
| `stringify_ids(**ids)` | Convert id/mst to strings | 4 × 12 = 48 |
| `execute_search(...)` | Common search logic | 40 × 8 = 320 |
| `execute_service(...)` | Common service logic | 30 × 10 = 300 |

### Refactored Tool Example

**Before (80 lines):**
```python
@server.tool(annotations=ToolAnnotations(...))
def eflaw_search(query, display, ..., ctx):
    """Search current laws..."""
    try:
        config = ctx.session_config if ctx else None
        session_oc = getattr(config, 'oc', None) if config else None
        resolved_oc = resolve_oc(...)
        # ... 40 more lines of param building, ranking, API call ...
    except ValueError as e:
        # ... 6 lines error handling ...
    except Exception as e:
        # ... 6 lines error handling ...
```
...
@with_error_handling("eflaw_search")
def eflaw_search(query, display, ..., ctx):
    """Search current laws..."""
    return execute_search(
        _get_client, ctx, oc, "eflaw", query, display, page, type,
        name_field="법령명한글",
        optional={"sort": sort, "ef_yd": ef_yd, "org": org, "knd": knd},
        date_params=["ef_yd"],
    )
```
...
|--------|--------|-------|--------|
| `server.py` lines | 3,124 | ~1,100 | **-65%** |
| `_helpers.py` lines | 0 | ~250 | new |
| **Total lines** | 3,124 | ~1,350 | **-57%** |
| Lines per search tool | ~80 | ~20 | -75% |
| Lines per service tool | ~60 | ~15 | -75% |

### Compatibility Preservation

| Platform | Requirement | How Preserved |
|----------|-------------|---------------|
| **MCP** | Tool discovery | Explicit `@server.tool()` on each function |
| **Smithery** | Context injection | `ctx: Context = None` in signatures |
| **Smithery** | Session config | `get_session_oc(ctx)` helper |
| **PlayMCP** | slim_response | Function unchanged |
| **LLM** | Type hints | Full signatures on tool functions |
| **LLM** | Docstrings | Kept on tool functions |

### Implementation Order

1. Create `_helpers.py` with all helper functions
2. Add imports to `server.py`
3. Refactor 8 search tools (eflaw_search, law_search, etc.)
4. Refactor 10 service tools (eflaw_service, law_service, etc.)
5. Refactor 4 linkage tools (lnkLs_search, etc.)
6. Handle `article_citation` separately (async)
7. Keep prompts as-is (already concise)
...
|------|-------|------------|
| MCP breaks | Low | Keep explicit decorators |
| Smithery breaks | Low | Keep Context parameter |
| Tests break | Medium | Run existing tests |
| Circular imports | Low | Local imports in helpers |

### Tool Categorization

**Search Tools (8) → `execute_search`:**
- eflaw_search, law_search, elaw_search, admrul_search
- prec_search, detc_search, expc_search, decc_search

**Service Tools (10) → `execute_service`:**
- eflaw_service, law_service, eflaw_josub, law_josub
- elaw_service, admrul_service, lsDelegated_service
- prec_service, detc_service, expc_service, decc_service

**Linkage Tools (4) → Custom:**
- lnkLs_search, lnkLsOrdJo_search, lnkDep_search, drlaw_search

**Special Tools (1):**
- article_citation (async, HTML parsing)

### Success Criteria

- [ ] All 24 tools still registered in FastMCP
- [ ] E2E tests pass
- [ ] Smithery Context injection works
- [ ] PlayMCP slim_response works
- [ ] No functionality changes

---

## Phase 6: Response Format Optimization (FUTURE)

**Status:** 📋 DEFERRED (evaluate after Phase 5)
**Objective:** Reduce LLM input tokens via optimized response formats

### Background

Current flow:
```
User Request → MCP Tool → XML Response → LLM Final Answer
```

Proposed optimization:
```
User Request → MCP Tool → XML → TOON/JSON → LLM Final Answer
```

### TOON (Token-Oriented Object Notation)

**What:** Compact, LLM-optimized serialization format (30-60% token reduction for tabular data)

**Example:**
```
# XML (~800 tokens)
<law><법령ID>123</법령ID><법령명>건축법</법령명></law>
<law><법령ID>124</법령ID><법령명>민법</법령명></law>
...
| Tool Type | TOON Savings | Recommendation |
|-----------|--------------|----------------|
| Search tools (8) | 50-60% | ✅ Good fit |
| Citation tool (1) | 40-50% | ✅ Good fit |
| Service tools - metadata | 30-40% | ⚠️ Moderate |
| Service tools - 본문 (body) | 5-15% | ❌ Minimal gain |

**Key insight:** Long document bodies (판례 본문, 법령 본문) are plain text - no format overhead to eliminate. TOON benefits are concentrated in **tabular search results**.

### Decision Factors

**Pros:**
- Significant token savings for search results
- No TOON format description needed (self-documenting headers)
- LLMs parse TOON well (74% accuracy vs JSON 70%)

**Cons:**
- Mixed format complexity (some tools TOON, others XML/JSON)
- Additional dependency (`python-toon`)
- Debugging with multiple formats
- Marginal gains for service tools (본문 dominates)

### Implementation Options (When Ready)

1. **TOON for search tools only** - Maximum benefit, accept mixed formats
2. **Unified slim JSON** - Optimize dict structure, skip TOON
3. **Hybrid** - TOON-encode array fields within JSON wrapper

### Prerequisites

- [ ] Phase 5 refactoring complete
- [ ] Benchmark current token usage per tool type
- [ ] Test LLM parsing accuracy with Korean legal text in TOON
- [ ] Evaluate maintenance burden vs. token savings

---

## Future Expansion Beyond Phase 6
...
**Total Future Potential:** 60+ additional tools beyond Phase 6

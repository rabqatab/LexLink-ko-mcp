...
> **범위:** All 24 implemented MCP tools across Phases 1, 2, 3, and 4
> **Status:** ✅ All 24 APIs implemented and validated (100%)
...
- **Phase 4 (1 tool):** Article citation extraction (HTML-based, 100% accuracy)
...
## 24) 조문 인용 추출 API (article_citation)
...
### Request Parameters
```yaml
mst: string (required)              # 법령일련번호 (법령 MST, eflaw_search 결과에서 획득)
law_name: string (required)         # 법령명 (예: "건축법", "자본시장과 금융투자업에 관한 법률")
article: int (required)             # 조문 번호 (예: 3 for 제3조)
article_branch: int (optional)      # 조문 가지번호 (예: 2 for 제3조의2, default: 0)
oc: string (optional)               # 사용자 ID (현재 citation에서 미사용)
```
...
internal_count: int                  # 내부 인용 개수 (같은 법령 내 다른 조문)
...
The tool extracts citations from law.go.kr HTML using CSS class mapping:

| CSS Class | Element Type | Example |
|-----------|--------------|---------|
| `sfon1` | Law name | 「형법」 |
| `sfon2` | Article | 제20조 |
| `sfon3` | Paragraph | 제1항 |
...
The `article_citation` tool is designed to be used after `eflaw_search`:

```
User: "건축법 제3조가 인용하는 법령을 알려줘"

1. eflaw_search(query="건축법")
   → MST: 268611, 법령명: 건축법

2. article_citation(mst="268611", law_name="건축법", article=3)
   → 12개 인용 (외부 8개, 내부 4개)

3. Response: "건축법 제3조는 총 12개 법령을 인용합니다:
   - 「국토의 계획 및 이용에 관한 법률」 제56조제1항
   - 「농지법」 제32조
   - ..."
```

### MCP Prompts for Citation

Two prompts are provided for automated citation workflows:

#### `get-article-with-citations`
**Purpose:** Get article content AND citations in one workflow
**Parameters:** `law_name`, `article_number`

#### `analyze-law-citations`
**Purpose:** Analyze citation networks for multiple articles
...
{
    "success": false,
    "errors": ["Law '존재하지않는법' not found"]
}

# Article not found
{
    "success": false,
    "errors": ["Article 999 not found in 건축법"]
}
...
# No citations
{
    ...
    "citation_count": 0,
    "citations": [],
    "message": "No citations found in this article"
}
```
...
1. **ID Mapping**: law.go.kr uses different IDs:
   - XML API uses `MST` (법령일련번호)
   - HTML pages use `lsiSeq` (법령ID)
   - The tool automatically converts MST → lsiSeq

2. **Citation Consolidation**: Sequential HTML links (law → article → paragraph) are merged into single citation objects

3. **Server Instructions**: The LexLink server includes embedded instructions that encourage LLMs to always use `article_citation` when discussing specific law articles